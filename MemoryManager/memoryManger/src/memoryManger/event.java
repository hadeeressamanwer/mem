...
public class event {
	...
	private   Vector<Segment> segmentV=new Vector<Segment>();
	...
	public void initiallization()  //H[2 3][10 2]
	{
		Hole h;
		h=holeq.peek(); //h=[2 3]
		...
		for(int j=0;j<memory.size();j++)
				{
			        ...
			        System.out.println(memory.get(j).segmentName);
			        System.out.println(memory.get(j).processName);
					System.out.println(memory.get(j).start);
					System.out.println(memory.get(j).size);
					System.out.println(memory.get(j).end);
					System.out.println(memory.get(j).busy);
				}
	...
	public void deallocateProcess(String processName) //remove segments
	{
		for(int i=0;i<memory.size();i++)
		{
			if(memory.get(i).busy&&memory.get(i).processName==processName)
			{
